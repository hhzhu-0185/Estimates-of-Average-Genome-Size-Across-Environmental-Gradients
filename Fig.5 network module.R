...
ko_module1_mean <- aggregate(cbind(pH, value) ~ variable, data = ko_module1, FUN = mean)

p1 <- ggplot() + 
  geom_smooth(data=ko_module1, aes(x= pH, y= log(value+1), group = ko), col= "grey", method = "lm" , size = 0.05,  fill=NA)+
  geom_smooth(data=ko_module1_mean, aes(x= pH, y= log(value+1) ), col= "red", method = "lm", size = 5)+
  labs(x = "pH",y = "Log (abundance + 1)")+ylim(-1,10)+
  theme_bw()+
...
lme0<-lme(value~pH,random=~1|ko,data=ko_module1, control=lmeControl(opt = "optim"))
summary(lme0)
...
ko_module2 <- ko_eukrep_vibrant1[ko_eukrep_vibrant1$ko %in% module[module$c.membership.fun.fc.. == "2", "ko"], ]
ko_module2 <- left_join(ko_module2, env, by=c("variable" = "site"))

ko_module2_mean <- aggregate(cbind(pH, value) ~ variable, data = ko_module2, FUN = mean)

p2 <- ggplot() + 
  geom_smooth(data=ko_module2, aes(x= pH, y= log(value+1), group = ko), col= "grey", method = "lm" , size = 0.05,  fill=NA)+
  geom_smooth(data=ko_module2_mean, aes(x= pH, y= log(value+1) ), col= "blue", method = "lm", size = 5)+
  labs(x = "pH",y = "Log (abundance + 1)")+ylim(-1,10)+
  theme_bw()+
  theme(plot.margin = margin(0.1,0.5,0.1,0.1, unit = "cm"),
        legend.title = element_text(colour="black", size=12, face="bold"),
...
lme0<-lme(value~pH,random=~1|ko,data=ko_module2, control=lmeControl(opt = "optim"))
summary(lme0)
...
ko_eukrep_vibrant2 <- aggregate(ko~level2 + c.membership.fun.fc.., data = ko_eukrep_vibrant2[,c(41,42,44)], FUN = function(x) length(unique(x)))
...
results_sorted <- results %>%
  ...
  mutate(sig = ifelse(logFC > 0.5, "Down", 
                      ifelse(logFC < - 0.5, "Up", "Stable")))
set.seed(123)
p3 <- ggplot(results_sorted, aes(x=-logFC, y=-log10(FDR), color = sig)) + 
  ...
  geom_hline(yintercept = -log10(0.02),lty=2,col="grey",lwd=0.5) +
  ...
  geom_text_repel(
    ...
    box.padding = 0,     # 增加文字与周围空间的距离
    point.padding = 0,   # 增加文字与数据点的距离
    segment.size = 0,    # 连接线细一点，减少视觉干扰
...
p1 <- ggplot() + 
  geom_smooth(data=ko_module1, aes(x= pH, y= log(value+1), group = ko), col= "grey", method = "lm" , size = 0.05,  fill=NA)+
  geom_smooth(data=ko_module1_mean, aes(x= pH, y= log(value+1) ), col= "blue", method = "lm", size = 5)+
  labs(x = "pH",y = "Log (abundance + 1)")+ylim(-1,10)+
  theme_bw()+
  theme(plot.margin = margin(0.1,0.5,0.1,0.1, unit = "cm"),
        legend.title = element_text(colour="black", size=12, face="bold"),
...
ko_module2 <- ko_kraken1[ko_kraken1$ko %in% module[module$c.membership.fun.fc.. == "2", "ko"], ]
ko_module2 <- left_join(ko_module2, env, by=c("variable" = "site"))
...
p2 <- ggplot() + 
  geom_smooth(data=ko_module2, aes(x= pH, y= log(value+1), group = ko), col= "grey", method = "lm" , size = 0.05,  fill=NA)+
  geom_smooth(data=ko_module2_mean, aes(x= pH, y= log(value+1) ), col= "red", method = "lm", size = 5)+
  labs(x = "pH",y = "Log (abundance + 1)")+ylim(-1,10)+
  theme_bw()+
  theme(plot.margin = margin(0.1,0.5,0.1,0.1, unit = "cm"),
        legend.title = element_text(colour="black", size=12, face="bold"),
...
ko_kraken2 <- aggregate(ko~level2 + c.membership.fun.fc.., data = ko_kraken2[,c(41,42,44)], FUN = function(x) length(unique(x)))
